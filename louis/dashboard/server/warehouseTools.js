...
2. 반드시 TOP N을 사용해 결과 행 수를 제한하세요 (기본 100, 집계/요약 질문이면 더 적게).
3. 여러 테이블에 걸친 조인이 필요하면, 위 목록에 있는 테이블 중에서만 조인하세요. 확신이 없으면 조인 없이 단일 테이블로 답변 가능한 형태로 좁히세요.
4. 이 목록의 테이블/컬럼으로 답할 수 없는 질문이면 reject_query를 호출하세요.
5. title은 결과를 설명하는 짧은 한국어 문구로 작성하세요.`
}
