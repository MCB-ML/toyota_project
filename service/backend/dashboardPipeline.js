...
import { applyDashboardPresentationPatch } from './dashboardObjectPatch.js'
...
const ACTION_BY_TOOL = {
  ...
  propose_patch_widget: 'presentation_patch',
...
function summarizeProposal(action, proposal, queryInfo) {
  ...
  if (action === 'presentation_patch') return `위젯 표시 방식 변경 (id=${proposal.widgetId})`
  return ''
}
...
export async function runDashboardPipeline({ message, history, dashboardState, modelId = null }, { sendEvent }) {
  ...
  const proposal = {
    ...
    size: call.args.size,
    presentation: call.args.presentation,
  }
  ...
  } else if (action === 'presentation_patch') {
    stage('patch')
    const existing = dashboardState.widgets.find((candidate) => candidate.id === proposal.widgetId)
    const result = applyDashboardPresentationPatch(existing, proposal.presentation)
    if (!result.ok) {
      sendEvent({ type: 'error', message: result.reason })
      return
    }
