...
export async function handleChatRequest(req, res, { dataDir }) {
  ...
  try {
    const made = createLlmClient(body?.modelId)
    if (!made) {
      sendEvent({ type: 'error', message: missingConfigMessage(body?.modelId) })
      return res.end()
    }
    const { client, model: chatModel } = made
