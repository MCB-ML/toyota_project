// Structural validator for SemanticQueryIR — live ESM port of
// agentic_bi_design/app/semantic/ir_schema.js. See that file's header comment
// for why this is hand-rolled instead of zod/ajv (no such dependency in this
// repo; server/dashboardValidation.js already uses the same style).
...
const TIME_GRAINS = new Set(['day', 'month'])
const ACCUMULATIONS = new Set(['none', 'running_total'])
const CHART_TYPES = new Set(['auto', 'bar', 'line', 'combo', 'table', 'donut'])
const TREND_CALCULATIONS = new Set(['actual', 'mom_change_rate'])
...
function validateTimeRange(tr, errors) {
  ...
  if (tr.type === 'relative') {
    ...
    if (tr.anchor_date !== 'runtime_context') {
      fail(errors, `${path}.anchor_date`, "LLM이 날짜를 직접 계산해 넣으면 안 됨 — 'runtime_context' 리터럴만 허용")
    }
  }
  if (tr.type === 'absolute') {
...
export function validateSemanticQueryIR(ir) {
  const errors = []
  ...
  if (!Array.isArray(ir.metrics) || ir.metrics.length === 0) {
    ...
    if (ir.metrics.length > 6) fail(errors, 'metrics', 'metrics는 최대 6개')
  ...
  if (Array.isArray(dimensions) && dimensions.length > 4) fail(errors, 'dimensions', 'dimensions supports at most 4 fields')
  if (Array.isArray(dimensions) && new Set(dimensions).size !== dimensions.length) fail(errors, 'dimensions', 'duplicate dimension request')
  ...
  if (ir.time_grain !== undefined && !TIME_GRAINS.has(ir.time_grain)) {
    fail(errors, 'time_grain', `${[...TIME_GRAINS].join('|')} 중 하나`)
  }

  if (ir.accumulation !== undefined && !ACCUMULATIONS.has(ir.accumulation)) {
    fail(errors, 'accumulation', `${[...ACCUMULATIONS].join('|')} 중 하나`)
  }

  if (ir.chart_type !== undefined && !CHART_TYPES.has(ir.chart_type)) {
    fail(errors, 'chart_type', `${[...CHART_TYPES].join('|')} 중 하나`)
  }

  if (ir.trend_calculation !== undefined && !TREND_CALCULATIONS.has(ir.trend_calculation)) {
    fail(errors, 'trend_calculation', `${[...TREND_CALCULATIONS].join('|')} 중 하나`)
  }

  if (ir.series_dimension !== undefined && !ID_PATTERN.test(ir.series_dimension)) {
    fail(errors, 'series_dimension', 'dimension id 형식 오류')
  }
  ...
  if (ir.intent === 'composite_analysis') {
    fail(errors, 'intent', 'composite_analysis는 AnalysisPlanner가 단일 분석들로 쪼갠 뒤여야 함')
  }
