...
function categoryCount(rows, field) {
  if (!field) return rows.length
  return new Set((rows || []).map((row) => JSON.stringify(row?.[field] ?? null))).size
}
...
export function validateDashboardIr(dashboardIr, ctx) {
  ...
  const categoryFieldsByMetric = ctx?.categoryFieldsByMetric || {}
  ...
  for (const comp of dashboardIr.components) {
    ...
    const categories = categoryCount(result?.rows, categoryFieldsByMetric[comp.metric])
    if (result?.rows && compDef?.max_categories && categories > compDef.max_categories) {
