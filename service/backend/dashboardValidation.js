...
import { validateDashboardObject } from '../frontend/src/utils/dashboardObject.js'

export const MAX_WIDGETS = 12

function looksLikeDashboardObject(widget) {
  return Boolean(
    widget?.id !== undefined ||
    widget?.chartCode !== undefined ||
    widget?.queryBundle !== undefined ||
    widget?.sql !== undefined ||
    widget?.sqlQueries !== undefined ||
    widget?.objectSpec !== undefined ||
    widget?.objectType !== undefined
  )
}
...
export function validateProposal(proposal, currentState) {
  ...
  if (['remove', 'modify', 'presentation_patch', 'reorder', 'resize'].includes(proposal.action)) {
...
export function validateWidgetProps(widget) {
  if (looksLikeDashboardObject(widget)) {
    const objectIssues = validateDashboardObject(widget)
    if (objectIssues.length) return { ok: false, reason: objectIssues.join(' ') }
  }
