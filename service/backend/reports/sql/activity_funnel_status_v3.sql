...
/* ── 파라미터 ──
   원본 GOLD의 DECLARE 블록을 제거했다. 이 변수들은 mssql 드라이버가
   request.input()으로 바인딩해 주입한다(문자열 치환 금지 — SQL 인젝션 방지).
   바인딩되는 것: @Year @MonthNumber @Brand @DealerNm @GroupName @DeptNm @ActYn @ScName @CommonTpNm
   @metric은 원본에서도 본문에 참조가 없어 삭제했다 — 컬럼 선택은 projection 단계에서 처리한다.

   원본과 다른 곳은 딱 두 군데뿐이고 둘 다 로직이 아니다:
     (1) 위 파라미터 DECLARE 제거 (바인딩으로 대체)
     (2) @*Pad 7개에 CASE NULL 가드 추가 — Fabric에서 N',' + NULL + N',' 가 NULL이
         아니라 ',,' 가 되는 탓에 "NULL=전체"가 "NULL=결과 없음"으로 뒤집히는 문제.
         (라이브 실행으로 확인: 가드 없으면 elig_user 0명 → 전체 리포트가 빈 결과)
   그 밖의 모든 줄은 원본 그대로다. server/reports/reports.test.js가 원본과 대조해 고정한다. */
...
BEGIN
    ...
    elig_user AS (
        ...
        WHERE ISNULL(u.facade_sc_yn, N'') <> N'창구SC'
          ...
          AND (@DealerNmPad  IS NULL OR @DealerNmPad  LIKE N'%,' + LTRIM(RTRIM(dlr.dealer_nm)) + N',%')
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ● 계약목
   (이하 원본 파일이 잘려 있어 설명이 중단됨 — 주석 종료를 보충한다) */