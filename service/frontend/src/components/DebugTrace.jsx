      ...
      <div className="flex flex-col gap-2 px-2.5 pb-2.5">
        {entries.map((e, i) => (
          <div key={i} className="border-t border-gray-200 pt-2">
            ...
            {/* agenticBiPipeline.js는 detail을 객체가 아니라 문자열(SQL 원문, IR JSON,
                자유 텍스트 메모)로 그대로 보낸다 — 위 객체 필드(topic/tables/sql) 중
                아무 것도 매치되지 않아 이전에는 라벨만 보이고 내용이 통째로 비어있었다. */}
            {typeof e.detail === 'string' && (
              <pre className="whitespace-pre-wrap break-all bg-white border border-gray-200 rounded p-1.5 mt-0.5 font-mono text-gray-700">{e.detail}</pre>
            )}
          </div>
        ))}
      </div>
