...
import KtwsHtmlReport from './pages/ktws/HtmlReport'
...
export default function App() {
  ...
  // /ktws/custom은 더 이상 여기 해당하지 않는다: 그 페이지가 Dynamic Semantic Query Planner
  // 콘솔을 직접 갖고 있어서(pages/ktws/Custom.jsx), 옆 챗봇까지 같은 일을 하면 같은 질문이
  // 두 경로로 갈린다. 그 탭의 떠 있는 챗봇은 일반 어시스턴트로 둔다.
  const chatPageKey = location.pathname === '/ktws/agentic-bi' ? 'agentic-bi' : undefined
  const { user } = useUser()
      ...
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto bg-[#f0f2f5] relative">
          <Routes>
            ...
            <Route path="/ktws/html-report" element={<KtwsHtmlReport />} />
        ...
        <ChatPanel
          ...
          title="AI 어시스턴트"
          subtitle={chatPageKey === 'agentic-bi' ? '대시보드 커스텀' : undefined}
          pageKey={chatPageKey}
        />
      </div>
