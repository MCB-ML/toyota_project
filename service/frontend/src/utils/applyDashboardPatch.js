...
import { normalizeDashboardObject, normalizeDashboardState } from './dashboardObject'

export function createEmptyDashboardState() {
  return normalizeDashboardState({ version: 0, widgets: [] })
}

export function applyPatchToState(state, patch) {
  let widgets = [...normalizeDashboardState(state).widgets]

  for (const op of patch?.ops || []) {
    if (op.op === 'add') {
      const index = op.index ?? widgets.length
      widgets.splice(index, 0, normalizeDashboardObject(op.widget))
    } else if (op.op === 'remove') {
      widgets = widgets.filter(w => w.id !== op.widgetId)
    } else if (op.op === 'update') {
      widgets = widgets.map(w => (w.id === op.widgetId ? normalizeDashboardObject(op.widget) : w))
    } else if (op.op === 'move') {
  ...
  return { version: (state.version || 0) + 1, widgets }
}
