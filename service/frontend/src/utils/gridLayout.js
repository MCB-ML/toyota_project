...
export function widgetToLayoutItem(widget) {
  // `layout` is the durable object-model contract. Older objects only have
  // left/top/right/bottom, so keep that form as a read fallback while every
  // new interaction writes the canonical layout too.
  const stored = widget.layout
  const hasStoredLayout = stored && [stored.x, stored.y, stored.w, stored.h]
  ...
  if (hasStoredLayout) {
    return {
      ...
      x: stored.x,
      y: stored.y,
      w: Math.min(GRID_COLS, Math.max(MIN_W, stored.w)),
      h: Math.min(MAX_H, Math.max(MIN_H, stored.h)),
      ...(Number.isInteger(stored.minW) ? { minW: stored.minW } : {}),
      ...(Number.isInteger(stored.minH) ? { minH: stored.minH } : {}),
      ...(Number.isInteger(stored.maxW) ? { maxW: stored.maxW } : {}),
      ...(Number.isInteger(stored.maxH) ? { maxH: stored.maxH } : {}),
    }
  }
